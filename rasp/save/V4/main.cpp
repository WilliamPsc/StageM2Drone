...
void calculs(Vehicle *vehicle)
{
	...
	mux.lock();
	std::cout << "Angle donné par drone : " << angle << " °" << std::endl;
	if (angle > 0 && angle <= 0.70)
	{
		angleReel = map(angle, 0.0, 0.70, 0, 90);
	}
	else if (angle > 0.70 && angle < 1.0)
	{
		angleReel = map(angle, 0.70, 1.0, 90, 180);
	}
	else if (angle < 0 && angle >= -0.70)
	{
		angleReel = map(angle, -0.70, -0.0, 270, 360);
	}
	else if (angle < -0.70 && angle > -1)
	{
		angleReel = map(angle, -1.0, -0.70, 180, 270);
	}
	else
	{
	...
	// Vecteur drone par rapport à l'axe des X (0,0)(cos ß, sin ß)
	double tabDrone[2][2];
	tabDrone[0][0] = 0;
	tabDrone[0][1] = 0;
	tabDrone[1][0] = cos(angleReel);
	tabDrone[1][1] = sin(angleReel);

	// Vecteur capteur par rapport au drone (0,0)( xp-xd, yp-yd )
	double tabCapteur[2][2];
	tabCapteur[0][0] = 0;
	tabCapteur[0][1] = 0;
	tabCapteur[1][0] = posCapteur[0] - posDrone[0];
	tabCapteur[1][1] = posCapteur[1] - posDrone[1];
	...
	pdtScalaire = tabCapteur[0][0] * tabDrone[0][0];
	pdtScalaire += tabCapteur[0][1] * tabDrone[0][1];
	pdtScalaire += tabCapteur[1][0] * tabDrone[1][0];
	pdtScalaire += tabCapteur[1][1] * tabDrone[1][1];

	// Division du produit scalaire par la distance
	double division = pdtScalaire / distance;
	std::cout << "pdtScal / distance : " << division << std::endl;
	...
	// moveByPositionOffset(vehicle, 0, 0, 2, 0); // Mouvement en Z
	// moveByPositionOffset(vehicle, 0, 0, 0, 0); // Mouvement en X
	// moveByPositionOffset(vehicle, 0, 0, 0, 0); // Mouvement en Y
...
void mesure(Vehicle *vehicle)
{
	while (stop)
	{
		...
		// magne          = vehicle->broadcast->getMag();
